...
char my_array[20] = "try proc array\n";
static int len = 1;
static struct proc_dir_entry *parent;

static int proc_open(struct inode *inode, struct file *file){
	pr_info("Procfs file opened successfully\n");
	return 0;
}

static ssize_t proc_read(struct file *filep, char __user *buffer, size_t length, loff_t* off)
{
	pr_info("Reading the proc file\n");
	
	/* Use the following condition to avoid reading 
	   mutliple times as we not taking caring of the offset */
	if(len){
		len = 0;
	}
	else{
...
static ssize_t proc_write(struct file *filep, const char *buffer, size_t len, loff_t* off)
{
	pr_info("Writing the proc file\n");
		
	//Clear the my_array before write as it is overlapping 
	memset(my_array,0,20);
	
	if(copy_from_user(my_array, buffer, len)){
		pr_err("Data writing failed\n");
	}
...
static int proc_close(struct inode *inode, struct file *file){
	pr_info("Procfs file closed successfully\n");
	return 0;
}
...
static long ioctl_drv(struct file *file,unsigned int cmd,unsigned long arg){
	switch(cmd){
		case WRITE_DRV:
		...
		case READ_DRV:
			...
			pr_info("value = %d read successfully", value);
			break;
		default:
			pr_info("In default case\n");
			break;
	}
	return 0;
...
static int fops_init(void){
	...
	parent = proc_mkdir("my_drv",NULL);
	
	if(parent == NULL){
		pr_err("Unable to create the proc entry\n");
		goto r_device;
	}
	
	proc_create("my_drv_proc", 0666, parent, &proc_fops);	
